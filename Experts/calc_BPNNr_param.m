function [best_hidden_size] = calc_BPNNr_param( k, Input_train, Output_Data,DrawingSwitch)
...
hidden_size0=size(Input_train0,2)-3:2*size(Input_train0,2);%%设定隐含层步长
...
%%生成优选数据集
HiddenSizeData=HiddenData(hidden_size0,2);
...
for hidden_size = 1:size(HiddenSizeData,1)
    ...
    for i = 1:k
        ...
        net = newff(actTrain_input, actTrain_output,HiddenSizeData(hidden_size,:));
