function [best_hidden_size] = calc_Elmanr_param( k, Input_train, Output_Data,DrawingSwitch)
...
hidden_size0=size(Input_train0,2)-3:2:2*size(Input_train0,2);%%设定隐含层步长
...
x_axis = size(Input_train0,2)-4:6*size(Input_train0,2);

%%生成优选数据集
HiddenSizeData=HiddenData(hidden_size0,3);
...
for hidden_size = 1:size(HiddenSizeData,1)
    ...
    for i = 1:k
        ...
        net = newelm(minmax(actTrain_input),[HiddenSizeData(hidden_size,:),1],{'tansig','tansig','tansig','purelin'},'traingdm','learngdm');
        net.trainParam.epochs = 1000;
        net.trainParam.goal = 0.0001;
        net.divideFcn='';
        net.trainParam.showWindow = false;
        net=init(net);
        net = train(net, actTrain_input, actTrain_output);
        y_val = net(validate_input);
        y_tra = net(actTrain_input);
...
save('calc_BPNNr_param.mat');
movefile('calc_BPNNr_param.mat', 'workfile');
end
